...
Permitted writes are managed output inside the configured vault and the broker's controlled root ignore additions. Approved build and sync scans also perform narrow project integration: owned `.claude/edw-doc/` files, a new root `CLAUDE.md` when no Claude entry point exists, or a supported instruction reference or rule adapter. Legacy owned integration is reused in place. Existing instructions, user edits, configuration, scripts, and hooks are preserved; default root ignore rules are `/edw-doc/` and `/.claude/`, plus `/CLAUDE.md` when setup creates it. Already tracked files remain tracked. Do not modify application source, tests, build files, credentials, `.claude` settings, Git configuration, Git hooks, or other ignored folders. An external reference is an unresolved boundary, not authorization to read it.
...
The broker performs integration only for approved build/sync command scopes and reports its status, changes, repairs, and warnings. Non-Git targets can skip integration with an explicit reason. Other analysis commands, plain static CLI scans, and hooks do not install it. Explicit CLI setup remains available for inspection and safe repair; uninstall is also a separate operator action. Never edit integration files directly or grant yourself native tools to bypass a refused setup. Report conflicts or skipped integration without claiming the project is fully configured.
