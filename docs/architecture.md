...
**Local installer:** approved build and sync scans invoke the same constrained installer available through the explicit CLI. It manages only owned instructions, configuration, references, and ignore additions within the selected Git root, and returns integration status to the workflow. Other analysis commands, static CLI scans, and hooks do not install integration. It preserves existing instructions, settings, and user changes. Ownership records support repeatable updates, repair of missing owned files through explicit setup, and selective uninstall. It never creates `CLAUDE.local.md` or exposes arbitrary file writes to agents.
...
Local setup routes a single eligible, locally ignored Claude instruction file through an owned import. Tracked, unignored, or ambiguous Claude entry points use an ignored rule adapter asking the host to read the exact instructions file. If neither root `CLAUDE.md` nor `.claude/CLAUDE.md` exists, setup creates an ignored root `CLAUDE.md`, preserving other instruction files such as `AGENTS.md` and `CLAUDE.local.md`. Scripts and hooks remain in the plugin, while existing project settings, scripts, and hooks remain intact. Status reports activation as unverified until checked in the host. See [installation](installation.md) for the decision table and preservation behavior.
