...
First call `mcp__plugin_doc-vault_vault__vault_begin` with `command: "onboard"` and `session_id: "${CLAUDE_SESSION_ID}"`. This is the single native approval for this invocation. If denied or unavailable, stop without retrying or using other tools. Pass the returned `run_id` unchanged to every later broker call, across all batches. Before your final response, call `mcp__plugin_doc-vault_vault__vault_end` with that ID, including when work is incomplete or fails. Do not reuse an ID from an earlier invocation.

Read `policies/core.md`, `workflows/onboard.md`, and `templates/onboarding.md` through `mcp__plugin_doc-vault_vault__vault_context`. Use the existing vault when current; inspect supporting source before relying on generated explanations.
