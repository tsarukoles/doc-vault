...
Read `policies/core.md` and `workflows/sync.md` through `mcp__plugin_edw-doc_vault__vault_context`. Use `vault_status` followed by `vault_refresh`, then inspect source changes, stale notes, and the returned `integration` result. This command's approval includes narrow broker-managed integration and ignore additions. Report integration changes, repairs, warnings, or a skipped-setup reason; preserve user instructions and configuration. Never bypass a setup refusal or claim file presence proves host activation.
